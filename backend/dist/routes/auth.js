...
router.post("/login", [
    ...
    (0, express_validator_1.check)("password", "Password with 6 or more chracter is required").isLength({ min: 6 })
    ...
    try {
        ...
        const isMatch = yield bcryptjs_1.default.compare(password, user.password); //take the password, form the same hashing as it did when the user created account and if the 2 hashing are the same, the password is correct
